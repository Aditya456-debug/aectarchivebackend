...
        }
        ...
        // ZERO TRUST CHECK: Validate Admin Approval
        if (!faculty.isAdminApproved) {
            return res.status(403).json({ success: false, msg: "ACCESS_DENIED: Awaiting Admin Approval." });
        }
...
// 🔥 NEW ZERO TRUST LOGIC: Request Access (From Faculty to Admin)
const requestAccess = async (req, res) => {
    try {
        const { facultyName, emailID, password, courses, isAdminApproved } = req.body;

        // Check if email already exists
        const existingFaculty = await Faculty.findOne({ emailID });
        if (existingFaculty) {
            return res.status(400).json({ success: false, msg: "EMAIL_ALREADY_EXISTS" });
        }

        // Create a temporary ID (will be replaced or formalized by Admin later if needed)
        const randomCode = Math.floor(1000 + Math.random() * 9000);
        const tempID = `REQ-${randomCode}`;
        ...
        const newRequest = new Faculty({
            ...
            courses: courses || [],
            isProfileCompleted: true, // They filled the form
            isAdminApproved: isAdminApproved || false // Waiting for Admin
        });

        await newRequest.save();

        res.status(201).json({ success: true, msg: "REQUEST_SENT_TO_ADMIN" });

    } catch (error) {
        console.error("❌ [REQUEST_ACCESS_ERROR]:", error);
        res.status(500).json({ success: false, msg: "Server Error sending request" });
    }
};
